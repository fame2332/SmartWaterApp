...
def calculate_harvest(precip_mm):
    """Calculates rainwater harvest in liters."""
    return precip_mm * AREA_M2 * (EFFICIENCY_COEFFICIENT / 1000)  # Convert mm to meters

# --- Fog Likelihood Prediction ---
# Load the fog prediction model (make sure you have 'fog_prediction_model.pkl')
try:
    with open('fog_prediction_model.pkl', 'rb') as model_file:
        loaded_data = pickle.load(model_file)
        if isinstance(loaded_data, dict):
    ...
    print("Error: 'fog_prediction_model.pkl' not found. Make sure the model file exists in the same directory.")
    fog_model = None  # Or handle this error appropriately in your application
    scaler = None
    optimal_threshold = None
def calculate_harvest(precip_mm):
    """Calculates rainwater harvest in liters."""
    return precip_mm * AREA_M2 * (
        EFFICIENCY_COEFFICIENT / 1000
...
try:
    with open("fog_prediction_model.pkl", "rb") as model_file:
        ...
        if isinstance(loaded_data, dict):
            ...
            raise ValueError(
                "Loaded data is not in the expected format. Ensure the model was saved as a dictionary."
            )
except FileNotFoundError:
    print(
        "Error: 'fog_prediction_model.pkl' not found. Make sure the model file exists in the same directory."
    )
    fog_model = None  # Or handle this error appropriately in your application
    scaler = None
    optimal_threshold = None
